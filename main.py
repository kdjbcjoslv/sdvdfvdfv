import os
import subprocess
import requests
import glob
import sys
import time
import re

# --- FUNCIONES DE UTILIDAD ---

def logger(mensaje, **kwargs):
    """Logs 100% anónimos para GitHub Actions."""
    print(mensaje, flush=True, **kwargs)

def limpiar_hashtag(nombre):
    """Limpia el nombre para el hashtag de Telegram."""
    nombre = nombre.lstrip('@')
    tag = re.sub(r'[^a-zA-Z0-9_]', '_', nombre)
    tag = re.sub(r'_+', '_', tag)
    return tag.strip('_')

def eliminar_id_de_archivo(video_id, archive_path="archive.txt"):
    """Libera el ID en el historial si el envío falla."""
    if not os.path.exists(archive_path): return
    linea_a_eliminar = f"tiktok {video_id}"
    try:
        with open(archive_path, "r") as f:
            lineas = f.readlines()
        with open(archive_path, "w") as f:
            for linea in lineas:
                if linea.strip() != linea_a_eliminar:
                    f.write(linea)
        logger("    ♻️ Archivo liberado en el historial para reintento.")
    except Exception:
        pass

def enviar_con_reintento(tipo, path, caption):
    """Envío con gestión de errores 429 y 400."""
    metodo = "sendVideo" if tipo == "video" else "sendPhoto"
    file_key = "video" if tipo == "video" else "photo"
    url = f"https://api.telegram.org/bot{TOKEN}/{metodo}"
    
    intentos = 0
    while intentos < 2:
        try:
            with open(path, 'rb') as f:
                payload = {'chat_id': CHAT_ID, 'caption': caption, 'parse_mode': 'HTML'}
                files = {file_key: f}
                r = requests.post(url, data=payload, files=files)
                if r.status_code == 200: return True
                if r.status_code == 429:
                    espera = r.json().get('parameters', {}).get('retry_after', 20)
                    logger(f"    ⚠️ Límite alcanzado. Esperando {espera}s...", end="")
                    time.sleep(espera + 1)
                    intentos += 1
                    continue 
                return False
        except Exception:
            return False
    return False

# --- CONFIGURACIÓN ---

TOKEN = os.getenv("TELEGRAM_TOKEN")
CHAT_ID = os.getenv("TELEGRAM_CHAT_ID")
USUARIOS_RAW = os.getenv("LISTA_USUARIOS", "")
USUARIOS = [u.strip() for u in USUARIOS_RAW.replace('\n', ',').split(",") if u.strip()]

logger(f"--- 🛠️ DEBUG MODO NINJA (Privacidad Total) ---")
logger(f"Analizando {len(USUARIOS)} cuentas configuradas.")

if not os.path.exists("temp_media"):
    os.makedirs("temp_media")

# --- PROCESO ---

for i, user in enumerate(USUARIOS, 1):
    # Log anónimo: solo número de usuario
    logger(f"\n👤 [Usuario #{i}] Analizando...") 
    
    tiktok_user = user if user.startswith('@') else f'@{user}'
    user_hashtag = limpiar_hashtag(user)
    
    subprocess.run([
        'yt-dlp', '--quiet', '--no-warnings',
        '--download-archive', 'archive.txt',
        '--dateafter', 'now-4day',
        '--playlist-end', '20',
        '--impersonate', 'chrome',
        '-o', 'temp_media/%(id)s.%(ext)s', 
        f'https://www.tiktok.com/{tiktok_user}'
    ])

    archivos = sorted(glob.glob("temp_media/*"))
    
    for j, file_path in enumerate(archivos, 1):
        video_id = os.path.splitext(os.path.basename(file_path))[0]
        ext = os.path.splitext(file_path)[1].lower()
        
        if ext in ['.m4a', '.mp3', '.wav', '.ogg']:
            os.remove(file_path)
            continue

        es_video = ext in ['.mp4', '.webm', '.mov']
        es_foto = ext in ['.jpg', '.jpeg', '.png', '.webp']
        if not es_video and not es_foto:
            if os.path.exists(file_path): os.remove(file_path)
            continue

        tipo = "video" if es_video else "photo"
        # La información sensible solo vive aquí (en el envío a Telegram)
        caption = f'🎬 Nuevo de: <a href="https://www.tiktok.com/{tiktok_user}">{user}</a>\n\n#{user_hashtag}'

        if es_foto: time.sleep(1.5)

        # Log anónimo: solo número de archivo
        logger(f"  📦 Enviando archivo #{j}...", end="")
        
        exito = enviar_con_reintento(tipo, file_path, caption)
        
        if exito:
            logger(" ✅")
            if os.path.exists(file_path): os.remove(file_path)
        else:
            logger(" ❌")
            eliminar_id_de_archivo(video_id)
            if os.path.exists(file_path): os.remove(file_path)

logger("\n--- ✨ Proceso terminado ---")
